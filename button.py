...
class Button(Label):
    ...
    def __init__(self, position, text, colour, background_colour, font_size, command):
        ...
        self.darkened_background = (max(background_colour[0] - 25, 0), max(background_colour[1] - 25, 0), max(background_colour[2] - 25, 0))
    ...
    def get_input(self, event):
        if event.type == pygame.MOUSEBUTTONUP:
            if event.button == 1 and self.get_rect().collidepoint(pygame.mouse.get_pos()):
                self.activated = True